...
def drop_one_step(x: int, y: int) -> (int, int, bool):
    ...
    tries = [(0, 1), (-1, 1), (1, 1)]
    for dx, dy in tries:
        if (x + dx, y + dy) not in cave:
            return x + dx, y + dy, True
    return x, y, False


def drop_sand(x: int, y: int) -> bool:
    """Drop sand into cave at parm position. Return True if it falls into abyss, False otherwise."""
    can_move, in_abyss = True, False
    while can_move and not in_abyss:
...
lx, ly, mx, my = 1000, 1000, -1000, -1000
for x, y in cave:
    lx = min(lx, x)
    ly = min(ly, y)
    mx = max(mx, x)
