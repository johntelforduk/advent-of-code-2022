...
def total_pressure(visit_order: list) -> int:
    ...
    for next_valve in visit_order:
        distance = shortest(from_valve=current_valve, to_valve=next_valve)
...
while True:
    random.shuffle((try_valves))
    best = total_pressure(try_valves)
    same_run = 0
    while same_run < 1000:
        i, j = random.randrange(len(try_valves)), random.randrange(len(try_valves))
        try_valves = swap(try_valves, i, j)
        new_pressure = total_pressure(try_valves)
        if new_pressure > best:
            best = new_pressure
            very_best = max(very_best, best)
            same_run = 0
        else:
            same_run += 1
            try_valves = swap(try_valves, i, j)

        print(very_best, best, same_run)
